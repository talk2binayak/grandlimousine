...
if ( ! class_exists( 'iworks_rate' ) ) {
	class iworks_rate {
		...
		private function get_plugin_data_by_plugin_id( $plugin_id ) {
			...
			$plugin['url']         = esc_url(
				sprintf(
					_x( 'https://wordpress.org/plugins/%s', 'plugins home', 'fleet' ),
					$plugin['slug']
				)
			);
			$plugin['support_url'] = esc_url(
				sprintf(
					_x( 'https://wordpress.org/support/plugin/%s', 'plugins support home', 'fleet' ),
					$plugin['slug']
				)
			);
			return $plugin;
		...
		public function get_advertising_og_content() {
			$args = array(
				...
				'plugin_name'        => __( 'OG — Better Share on Social Media', 'fleet' ),
				'plugin_wp_home'     => __( 'https://wordpress.org/plugins/og/', 'fleet' ),
			);
